...
X_test <- read.table(file.path(dataPath,"household_power_consumption.txt"), header = TRUE, sep = ";", stringsAsFactors = FALSE )
...
par(mfrow = c(2,2))
plot(sampledata$Global_active_power, type = 'l', mfrow = 2, mfcol = 2, xlab = "", ylab = "Global Active Power") 
plot(sampledata$Voltage, type = 'l', mfrow = 2, mfcol = 2, xlab = "datetime", ylab = "Voltage") 
plot(sampledata$Sub_metering_1, type = 'l', col = 'black', xlab = "", ylab = "Energy sub metering")
points(sampledata$Sub_metering_2, type = 'l', col = 'red')
points(sampledata$Sub_metering_3, type = 'l', col = 'blue', ylab = "Energy sub metering")
legend("topright",c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),lty=c(1,1),lwd=c(2.5,2.5),col=c("black","red","blue"))
plot(sampledata$Global_reactive_power, type = 'l', mfrow = 2, mfcol = 2, xlab = "datetime", ylab = "Global_reactive_power") 
dev.off()
